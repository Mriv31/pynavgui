...
class dsStyleMenu(QtWidgets.QMenu):
    def __init__(self, title, parent, PDs):
        ...
        self.addMenu(
            PointContourColorMenu(
                "Change Point Contour Color", parent=self, dataset=PDs
            )
        )
        self.addMenu(
            PointFillingColorMenu(
                "Change Point Filling Color", parent=self, dataset=PDs
            )
        )
    ...
    def __init__(self, title, parent, dataset):
        ...
        for c in colors:
            ...
            coloraction.triggered.connect(
                lambda _, y=c: dataset.change_point_fill_color(y)
            )
